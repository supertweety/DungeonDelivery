...
Use `dungeon_delivery.pygame_viewer.replay_result(result)` to open an animated pygame window for one completed `GameResult`, or `replay_tournament(tournament_result)` to choose among many rounds inside the pygame window. The viewer shows the map terrain, walls, available keys, doors, agents, available packages as yellow parcel icons, package destinations, current frame, current turn, the last action taken, and an in-window legend explaining the tile colors and overlays. The right side of the replay contains one panel per agent with that agent's score, collected keys, and currently carried package. These panels are separate from the fixed REPLAY, SELECT GAME, and EXIT buttons, so the buttons do not cover agent information. When a key is collected, it disappears from later replay frames and the cell is drawn as floor. The pygame window is resizable; if the board or side panel does not fit, visible scrollbars appear and the mouse wheel scrolls the board or the side panel depending on where the pointer is. Press Space to pause, Left/Right to step, R or REPLAY to restart, SELECT GAME to return to the menu, and Esc or EXIT to quit.
